...
import com.core.support.QueryResult;
        ...
        implements Dao<E>
{
    ...
    public List<E> doQuery(BaseParameter param) {
        ...
        try {
            if ((param.getSortedConditions() != null) && (param.getSortedConditions().size() > 0)) {
                ...
                for (Iterator it = param.getSortedConditions().keySet().iterator(); it.hasNext(); ) {
                    String pm = (String)it.next();
    ...
    protected void setParameter(Map<String, Object> mapParameter, Query query) {
        for (Iterator it = mapParameter.keySet().iterator(); it.hasNext(); ) {
    ...
    private void processQuery(Criteria criteria, BaseParameter param) {
        try {
            String prop;
            Map staticConditionMap =BeanUtils.describeAvailableParameter(param);
            Map dynamicConditionMap = param.getQueryDynamicConditions();
            Disjunction disjunction = Restrictions.disjunction();
            if ((staticConditionMap != null) && (staticConditionMap.size() > 0)) {
                for (Object o : staticConditionMap.entrySet()) {
                    //TODO  why  Map.Entity e :  staticConditionMap.entrySet()  has error?
                    Entry e = (Entry)o;
            ...
            if ((dynamicConditionMap != null) && (dynamicConditionMap.size() > 0)) {
                ...
                Iterator iterator;
                for (iterator = dynamicConditionMap.entrySet().iterator(); iterator.hasNext(); ) { e = (Map.Entry)iterator.next();
