...
public class Page<T>{
    ...
    private int pageSize =Integer.valueOf(Constant.props.get("pageSize").toString());     //页面显示的数目
    ...
    private boolean first=false;        //是否有上一页
    private boolean last=false;         //是否最后一页
    ...
    public int[] getPageBar() {
        ...
        int pageBar[] = null;
        if(this.getTotalPage() <= pageSize){    //当总页码不足或等于既定页面大小时
            pageBar = new int[this.totalPage];
            startPage = 1;
            endPage = this.totalPage;
        }else{                  //当总页码大于既定页面大小时
            pageBar = new int[pageSize];
            startPage = this.currentPage - (pageSize/2-1);    //为了保证当前页在中间
            endPage = this.currentPage + pageSize/2;
            ...
            if(endPage>this.totalPage){
                endPage = this.totalPage;
                startPage = this.totalPage - (pageSize-1);
            }
        }
