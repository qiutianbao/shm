...
//专门验证checkbox
$.validation.CheckBoxValidator =$.validation.Validator.inherit({
	acceptTypes: ['checkbox'],
    min: 0,
    max: 0,
    getMin: function() {
        return this.min || this.min >= 0 ? this.min : 0;
    ...
    doValidate: function(input, result) {
		var value = $("input[type=checkbox][name="+$(input).attr("name")+"]:checked").length;
        if (!value || value < this.getMin() || value > this.getMax()) {
            this.setError(result);
        }
    }
});
$.validation.validatorMgr.reg('checkbox', $.validation.CheckBoxValidator);

//Ajax
$.validation.AjaxCallValidator = $.validation.Validator.inherit({
    acceptTypes: ['text', 'password'],
...
$.validation.validatorMgr.reg('ajaxCall', $.validation.AjaxCallValidator);
